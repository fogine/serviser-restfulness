...
class QuerySegmentCollection extends Array {
    ...
    /**
     * @param {QuerySegmentCollection} collection
     * @return {Boolean}
     */
    equals(collection) {
        for (let i = 0, len = Math.max(this.length, collection.length); i < len; i++) {
            if (!(collection[i] instanceof QuerySegment)) {
                return false;
            }

            if (!collection[i].equals(this[i])) {
                return false;
            }
        }
        return true;
