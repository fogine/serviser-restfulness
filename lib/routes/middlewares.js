...
function parseEmbedParameterMiddleware(req) {
    ...
    //TODO _embed parameter should be allowed to contain associated resources
    //that are listed in route response schema.
    if (req.query._embed) {
...
function parseFilterParameterMiddleware(req) {
    ...
    if (routeUtils._hasResponseSchema(this.route)) {
        const responseSchema = routeUtils._getResponseSchema(this.route);
        columnWhitelist.push.apply(
            columnWhitelist,
    ...
    if (req.query._filter) {
        const invalidColumns = _.difference(
            Object.keys(req.query._filter),
            columnWhitelist
        );
        if (invalidColumns.length) {
            throw new RequestError(
                `Invalid _filter target(s) ${invalidColumns.join(',')}.`
            );
        } else {
            _setRestfulnessReqContext.call(this, {
