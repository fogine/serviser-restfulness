...
function _settleRouteResponseSchema(route, wrapIntoCollection) {
    if (!_hasResponseSchema(route)) {
        ...
        const properties = _clearValidationKeywordsExceptDataType(
        ...
        resource.getAssociatedResourceNames().forEach(function(name) {
            if (resource.hasAssociation(name, '1x1')) {
                ...
                const subProperties = _clearValidationKeywordsExceptDataType(
...
function _settleRouteValidators(route) {
    ...
    if (hasPayload && !_hasTargetValidator(route, 'body')) {
        const required = segment.resource.getRequiredProperties();
        const properties = _.clone(segment.resource.getProperties());

        if (firstSegment !== segment
            && firstSegment
            && segment.resource.hasAssociation(firstSegment.resource, '1x1')
        ) {
            //TODO make a unit test that will make sure the user of API cant overwrite
            //resource ids defined in params by those defined in body
            const association = segment.resource.getAssociation(firstSegment.resource);
...
function _acceptEmbedParameter(route) {
    route.validate({
        ...
        additionalProperties: true,
        properties: {
            _embed: {type: 'string', pattern: '^[a-z0-9.,_]+$'}
        }
    }, 'query');
 ...
 * @return {Object}
 */
function _clearValidationKeywordsExceptDataType(properties, ajv) {
    Object.keys(properties).forEach(function(prop) {
        ...
        if (schema.hasOwnProperty('$ref')) {
            ...
            properties[prop] = Object.assign(
                _.pick(refSchema, ['type', 'default']),
                _.omit(schema, ['$ref'])
            );
        } else {
            properties[prop] = _.pick(schema, ['type', 'default']);
        }
    });
