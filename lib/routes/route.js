...
function _settleRouteResponseSchema(route, wrapIntoCollection) {
    if (!_hasResponseSchema(route)) {
        ...
        const allowedValidationKeywords = ['type', 'default', 'nullable'];//TODO make unit tests for allowed keywords
...
function _settleRouteQueryValidator(route, resource) {
    ...
    let responsePropList;

    if (_hasResponseSchema(route)) {
        responsePropList = Object.keys(_.get(
           _getResponseSchema(route),
           'items.properties',
           {}
       ));
    } else {
        responsePropList = Object.keys(resource.getResponseProperties());
    }
    let properties = {};
    responsePropList.forEach(function(propName) {
    ...
    const keywordWhitelist = ['type', 'minimum', 'maximum',
        'maxLength', 'minLength', 'nullable', 'faker'];
...
function _acceptFilterParameter(route) {
    route.validate({
        ...
        additionalProperties: true,
        properties: {
            _filter: queryFilter.FILTER_JSON_SCHEMA,
        }
    }, 'query');
    ...
    route.step(
        '_restfulness_parse_filter',
        middlewares.parseFilterParameterMiddleware
    );
}
