...
function put(url, options) {
    ...
    process.nextTick(function() {
        ...
        if (!routeUtils._isImplemented(route)) {
            ...
            route.options.summary = `update a ${lastSegment.resource.getName()}`;
            ...
            if (querySegments.length === 1) {
                ...
                switch (association.type) {
                    case 'MxM':
                        ...
                        route.options.summary = `associate ` +
...
function updateViaManyToManyQuery(req, res) {
    ...
    return knex.transaction(function(trx) {
        ...
        return query.into(tableName).catch(routeUtils._transformKnexError);
    }).then(postProcess)
