...
function createViaOneToOneQuery(req, res) {
    ...
    const tableName = secondResource.getPluralName();
    ...
    .then(function (rows) {
        ...
        const location =  _getLocationHeader(route, locationParams);
        if (location) {
...
function createViaDistantOneToOneQuery(req, res) {
    ...
    return knex.transaction(function(query) {
        return query
            ...
            .from(firstResource.getPluralName()).then(function(rows) {
                if (!rows.length) {
...
function createViaManyToManyQuery(req, res) {
    ...
    return knex.transaction(function(query) {
        return query
            ...
            .then(function(rows) {
                ...
                return query
                    .insert(pivotData)
                    .into(association.through.resource.getPluralName())
    ...
    function postProcess(rows) {
        const locationParams = [
            pivotData[association.foreignKey],
            pivotData[association.localKey]
        ];
        const location =  _getLocationHeader(route, locationParams);
        if (location) {
            res.setHeader('Location', location);
...
function createViaDistantManyToManyQuery(req, res) {
    ...
    return knex.transaction(function(query) {
        return query
            ...
            .from(firstResource.getPluralName())
            .catch(routeUtils._transformKnexError)
            .then(function(rows) {
                if (!rows.length) {
                    throw new RequestError({
