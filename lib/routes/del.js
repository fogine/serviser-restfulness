...
function delViaSingleResourceQuery(req, res) {
    ...
    return query.del().catch(routeUtils._transformKnexError).then(function(count) {
        ...
        query.whereIn(pivotTableName + '.' + association.through.localKey, function() {
            ...
            if (lastSegment.isReduced()) {
                ...
                Object.keys(req.query).forEach(function(key) {
                    where[secondResource.getTableName() + '.' + key] = req.query[key];
                });
            }

            return this
                .from(secondResource.getTableName())
                .select(association.localKey)
                .where(where);
        });
    }
