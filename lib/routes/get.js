...
function get(url, options) {
    ...
    if (!lastSegment.isReduced()) {
        route.description.summary = `find ${lastSegment.resource.getPluralName()}`;
        ...
        route.description.summary = `get ${lastSegment.resource.getName()}`;
        route.Router.App.$setRoutesForResource(
    ...
    //_sort & _limit & _offset & _embed & _filter query validation failures
...
function getOneViaSingleResourceQuery(req, res) {
    ...
    query.modify(sqlUtils.withEagerLoad, embed, resource, responseSchema.properties);
...
function getOneViaOneToOneQuery(req, res) {
    ...
    //eager load user defined associated resources
    query.modify(sqlUtils.withEagerLoad, embed, secondResource, responseSchema.properties);
...
function getOneViaOneToManyQuery(req, res) {
    ...
    //eager load user defined associated resources
    query.modify(sqlUtils.withEagerLoad, embed, lastResource, responseSchema.properties);
...
function getManyViaManyToManyQuery(req, res) {
    ...
    query.modify(queryFilter.withFilter, filter, secondResource.getTableName());
    ...
    //eager load user defined associated resources
    query.modify(sqlUtils.withEagerLoad, embed, secondResource, responseSchema.items.properties);
...
function getManyViaOneToOneQuery(req, res) {
    ...
    query.modify(queryFilter.withFilter, filter, tableName);
...
function getManyViaSingleResourceQuery(req, res) {
    ...
    query.select(sqlUtils.generateKnexSelect(select, tableName));

    //eager load user defined associated resources
    query.modify(sqlUtils.withEagerLoad, embed, resource, responseSchema.items.properties);
