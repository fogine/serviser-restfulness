...
Filter.prototype._getTableColumn = function(column) {
    ...
    if (segments.length === 1) {
        ...
        if (!this._joinedTables.includes(resource.getTableName())) {
            ...
            if (this._resource.hasAssociation(resource, '1x1')) {
                this._knex.leftJoin(
                    resource.getTableName(),
                    `${this._tableName}.${association.localKey}`,
                    `${resource.getTableName()}.${association.foreignKey}`
                );
            } else if (this._resource.hasAssociation(resource, '1xM')) {
                this._knex.leftJoin(
                    resource.getTableName(),
                    `${this._tableName}.${association.localKey}`,
                    `${resource.getTableName()}.${association.foreignKey}`
                );
            } else if (this._resource.hasAssociation(resource, 'MxM')) {
                const pivotTableName = association.through.resource.getTableName();
                this._joinedTables.push(pivotTableName);
                this._knex.leftJoin(
                    pivotTableName,
                    `${this._tableName}.${association.localKey}`,
