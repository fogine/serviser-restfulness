...
Segment.prototype.reduceBy = function(property) {
    ...
    if (this._segmentCollection) {
        this._segmentCollection.uniquifyPropertyNames();
    }
};
...
Segment.prototype.equals = function(segment) {
    ...
    if (segment.resource !== this.resource) {
        return false;
    }

    if (segment.isReduced() !== this.isReduced()) {
        return false;
    }

    if (segment.isReduced()
        && segment.getOriginalPropertyName() !== this.getOriginalPropertyName()
    ) {
        return false;
    }
